...
function autoWrapLatex(text) {
    ...
    const textPatterns = [
        ...
        /^[A-Za-z]+\s+[A-Za-z]+\s+[A-Za-z]+/,                  // 3개 이상 단어
        /^(?:to|for|of|with|from|by|in|at|on|and|or|but|nor|for|yet|so)\s/i, // 전치사 시작
        /^[A-Z][a-z]+(?:[ ][A-Z][a-z]+)*[;:]/,                 // 콜론/세미콜론으로 끝나는 문장
        /^[A-Z][a-z]+[,]/,                                     // 쉼표로 이어지는 문장
        /^[A-Za-z]+['’][A-Za-z]+/,                            // 축약어 (don't, it's 등)
        /^How\s+[A-Za-z]/,                                     // "How"로 시작하는 질문
        /^What\s+[A-Za-z]/,                                    // "What"로 시작하는 질문
        /^Which\s+[A-Za-z]/,                                   // "Which"로 시작하는 질문
        /^Why\s+[A-Za-z]/,                                     // "Why"로 시작하는 질문
        /^When\s+[A-Za-z]/,                                    // "When"로 시작하는 질문
        /^Where\s+[A-Za-z]/,                                   // "Where"로 시작하는 질문
        /^[A-Z][a-z]+(?:[ ][A-Z][a-z]+)*\?$/,                  // 물음표로 끝나는 문장
        /^[A-Z][a-z]+(?:[ ][A-Z][a-z]+)*[.!?]$/,              // 마침표/물음표로 끝나는 문장
        /^[A-Z][a-z]+(?:[ ][A-Z][a-z]+)*\s+/,                  // 대문자로 시작하는 일반 문장
    ];
