...
function detectTrialUser(user) {
  if (!user || typeof user !== 'object') return false;
  var accountType = normalizeAccountValue(user.account_type || user.accountType || user.role);
  if (accountType === 'admin') return false;

  var paymentStatus = normalizeAccountValue(
    user.payment_status || user.paymentStatus || user.status || user.member_status
  );
  return paymentStatus === 'p' || paymentStatus === 'pending' || paymentStatus === 'trial';
}

function getQuestionRequestLimit() {
  return IS_TRIAL_USER ? TRIAL_QUESTION_LIMIT : QUESTIONS_PER_SET;
}
...
function applySubjectConfig() {
  ...
  IS_TRIAL_USER = detectTrialUser(currentUser);
  if (IS_TRIAL_USER) {
    QUESTIONS_PER_SET = TRIAL_QUESTION_LIMIT;
    TOTAL_QUESTIONS = TRIAL_QUESTION_LIMIT;
    subjectConfig.SET_SIZE = TRIAL_QUESTION_LIMIT;
    subjectConfig.QUESTION_COUNT = TRIAL_QUESTION_LIMIT;
    console.log('🧪 FREE TRIAL active: questions 1-' + TRIAL_QUESTION_LIMIT);
  }
...
async function load50Questions(uiStartNumber, retryCount = 0) {
    ...
    try {
        ...
        requestParams.set('limit', String(getQuestionRequestLimit()));
        ...
        if (IS_TRIAL_USER && processed.length > TRIAL_QUESTION_LIMIT) {
            processed = processed.slice(0, TRIAL_QUESTION_LIMIT);
        }
...
function resumeProgress(saved) {
  currentQuestions = Array.isArray(saved.currentQuestions) ? saved.currentQuestions.slice() : [];
  userAnswers = Array.isArray(saved.userAnswers) ? saved.userAnswers.slice() : [];

  // Trial 계정이 과거에 저장한 120문제 세션을 재개해도 20문제를 넘지 못하게 한다.
  if (IS_TRIAL_USER) {
    currentQuestions = currentQuestions.slice(0, TRIAL_QUESTION_LIMIT);
    userAnswers = userAnswers.slice(0, TRIAL_QUESTION_LIMIT);
    while (userAnswers.length < currentQuestions.length) userAnswers.push(null);
  }
...
async function startQuizWithNumber(uiStartNumber) {
  if (IS_TRIAL_USER) uiStartNumber = 1;
