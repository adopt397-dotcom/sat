...
function applyCopyProtectionPolicy() {
  ...
  function currentSessionIsAdmin() {
    if (IS_ADMIN_USER || document.documentElement.classList.contains('admin-user')) return true;
    try {
      var storedUser = JSON.parse(localStorage.getItem('quiz_current_user_v1') || 'null');
      return isAdminUser(storedUser);
    } catch (error) {
      return false;
    }
  }
  ...
  ['copy', 'cut', 'paste', 'contextmenu', 'selectstart'].forEach(function(type) {
    document.addEventListener(type, function(event) {
      // 관리자 계정은 복사, 붙여넣기, 우클릭, 텍스트 선택을 모두 허용한다.
      if (currentSessionIsAdmin()) return;
